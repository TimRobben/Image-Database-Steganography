...
for name, row in data.iterrows():
    ...
    dest_path = f'/media/tim/Backup Plus/Scriptie/Methods/SampleDB/{row.phone}/{image_name}'
    png_dest_path = f'/media/tim/Backup Plus/Scriptie/Methods/SampleDB/{row.phone}/{image_name}.png'
    secret_path = f'/media/tim/Backup Plus/Scriptie/Methods/Github Labels/code/2 - applying methods/messages/{row.message}'

    if row.method == 6:
        if row.embeddingRate == 0.5 and 47 < row.complexity < 63:
            DCT1.append(f"python3 DCT_hide.py '{png_path}' '{png_dest_path}' '{secret_path}'")

    elif row.method ==7:
        if row.embeddingRate == 0.5 and 50 < row.complexity < 60:
...
text_file = open("/media/tim/Backup Plus/Scriptie/Methods/SampleDB/commands/DCT1.txt", "w")
text_file.write(DCT1_commands[1:])
text_file.close()

text_file = open("/media/tim/Backup Plus/Scriptie/Methods/SampleDB/commands/DCT2.txt", "w")
text_file.write(DCT2_commands[1:])
text_file.close()

text_file = open("/media/tim/Backup Plus/Scriptie/Methods/SampleDB/commands/DCT3.txt", "w")
text_file.write(DCT3_commands[1:])
text_file.close()

text_file = open("/media/tim/Backup Plus/Scriptie/Methods/SampleDB/commands/DCT4.txt", "w")
text_file.write(DCT4_commands[1:])
text_file.close()

text_file = open("/media/tim/Backup Plus/Scriptie/Methods/SampleDB/commands/DCT5.txt", "w")
